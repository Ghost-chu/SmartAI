		...
		if(label.equals("smartai") && sender.hasPermission("smartai.admin")) {
			if(args.length==1) {
				...
				case "reload":
					...
					return true;
				case "status":
					sender.sendMessage("§aSmartAI status §b>> §dLast sync task usage time: §e"+lastSynctaskTime+"ms§d, Last async task usage time: §e"+lastASynctaskTime+"ms§d, Now have §e"+limitList.size()+" mobs §din control list, Mobs AI status now: §r"+getAIMode()+"§d, ServerTPS: §e"+tps);
					return true;
				default:
					return true;
				}
